"""Engagement policy on a FIXED transcript (CPU only; no model load).

    .venv/bin/python benchmarks/spec_engage_replay.py <model dir> <probe json> [drain_ms]

``<probe json>`` is a ``probe_spec_ngram_impl.py`` output: its ``off`` arm carries the
non-speculative token ids this replays. Engagement policy on a FIXED transcript — the comparison the throughput arms cannot make.
