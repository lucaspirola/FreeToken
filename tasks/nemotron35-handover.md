...
State as of 2026-09-05, HEAD `14c1bd8` on `main`, working tree clean. `fork/nemotron35` is at
the same commit (pushed); `fork/main` is 77 commits behind and 0 ahead — a fast-forward is
...
| metric | start (2026-09-04, ~`508ea32`) | now (`14c1bd8`) | evidence |
|---|---|---|---|
...
| prefill 131K | 3,230 tok/s | **5,882** (1.82x) | prefill_profile + moe_prefill_gemm |
...
1. **n-gram verify overhead** — ~40% of a verify step is not the forward (~52 ms vs a ~30 ms
   ...
   vs 0.353 offline). `ngram_spec_impl_2026-09-05.md` §6.
2. **`graph.py::_determine_cuda_graph_bs` has the same padding defect** on the NON-elastic path
   (`[1,2,4] + range(8,max_bs+1,8)`); a 12-lane batch padding to 16 measured **−6.7%** here.
   Blast radius is every model, so measure one dense model first — the experiment is written as
   `phaseE.sh`. `decode16_2026-09-05.md` §7.1.
3. **MoE A-operand vectorization** — `a_ptrs_lo`/`a_ptrs_hi` read the same span at a 2-element
   stride, so neither activation load vectorizes; a one-off deinterleave of A per GEMM is
   ~0.5 ms of HBM. Most likely remaining Triton-side win against b12x's 1.34x.
   `moe_prefill_gemm_2026-09-05.md` §10b.
4. **Extend-cache threshold vs the scheduler's 512-token chunks** — decidable in one run:
   per-chunk time and the following decode's miss rate at `--moe-extend-cache-tokens`
   64 / 512 / 2048 on a 131K prompt. Related: the M=256 GEMM bucket runs at 20% of ceiling with
   +53% padding waste at `BLOCK_M=16`. `extend_moe_2026-09-05.md`, `moe_prefill_gemm` §10e.
5. **16-way decode is at the hardware ceiling — do not re-litigate.** 74% of the step is the
...
6. **fork/main fast-forward** — user decision. `fork/main` (62f5a66) is a strict ancestor of
   HEAD; `fork/nemotron35` already carries the merge.
7. **`_gguf` extension rebuild before deploying on Ada** — the fork/main merge (`32cc504`)
   changed multiwarp bool → warps int64; a stale `.so` silently picks the 4-warp path.
8. **Oracle 131K rung** on both engines (~10 min total), and the 524K `direct:harbour` lead
...
9. **Scheduler tickets** (all in `tasks/todo.md` with evidence): an over-pool prompt has no
...
10. **Watch mean lanes per prefill batch** every soak: 3.43 stage / 4.92 passthrough at
    `13af13d`. Stage >~5 **together with** rising errors or p95 is the §R6/§R7 mode returning.
11. **CI follow-ups** — re-run the full unit-test step with no GPU job live and record the wall
...
12. **Pre-existing test issues, not this effort's** — `tests/server/test_muse_glimmer_parsers.py`
...
- **A/B hatches**: `FREETOKEN_DECODE_{KV_SPLITS,BLOCK_N,NUM_WARPS}`,
  ...
  `FREETOKEN_ELASTIC_GRAPH_MAX_BS`, `FREETOKEN_NEMOTRON_DT_MIN`, `FREETOKEN_MAMBA2_REF`.
