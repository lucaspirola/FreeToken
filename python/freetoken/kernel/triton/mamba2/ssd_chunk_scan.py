...
# Vendored from vLLM `vllm/model_executor/layers/mamba/ops/ssd_chunk_scan.py`
# (sequence-aligned varlen chunk variant, `seq_idx` per chunk + per-sequence
# `initial_states`), which itself adapts
# https://github.com/state-spaces/mamba/blob/v2.2.4/mamba_ssm/ops/triton/ssd_chunk_scan.py
#
# FreeToken changes: `vllm.triton_utils` -> plain triton, `fast_exp` from
# `ssd_chunk_state`; the 22-entry autotune list pruned to 8 sm_120-sized configs;
# `autotune_cache_kwargs`; `do_not_specialize` on the chunk-count strides; the
# unused `IS_TRITON_22` flag dropped (triton is pinned at 3.6 here).
...
# With chunk 128 / hdim 64 / dstate 128 the worst config below is 48 KB, well
# inside the ~100 KB an sm_120 block can hold. Every vLLM config with BLOCK_N
# 128/256 at stages 3-4 (up to 224 KB) is dropped, plus the redundant duplicates.
@triton.autotune(
    configs=[
        triton.Config(
...
def _chunk_scan_fwd_kernel(
    ...
    for k in range(0, K_MAX, BLOCK_SIZE_K):
        ...
        # Chunks never straddle a sequence boundary, so no seq_idx masking is
        # needed here. minimum(., 0) keeps the causal decay <= 1.
...
def _chunk_scan_fwd(
    ...
    """Diagonal blocks + carried state -> writes `out` (seqlen, nheads, headdim) in place."""
